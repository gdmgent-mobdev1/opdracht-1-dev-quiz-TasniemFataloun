...
const SetQuiz = () => {
  ...
  for (const key in currentQuestion.correct_answers) {
    ...
    multipleAnswers.textContent =
      countAnswers >= 2 ? "There is multiple Answers" : "";
  }
    ...
    for (const i in q.answers) {
      if (q.answers[i] !== null) {
        ...
        q.selectedAnswer.forEach((element:any) => {
          if (i == element) {
            ...
            if (q.correct_answers[`${element}_correct`] === "true") {
              ...
              answerItem.style.border = "10px green solid";
              ...
              answerItem.style.border = "10px red solid";
        ...
        /* if (i == q.selectedAnswer) {
          answerItem.style.color = "var(--primary-color";
        } */
      }
    }
